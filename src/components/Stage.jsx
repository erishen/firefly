...
//   萤火女仆      public/fireflyMaid.vrm      —— 用户导入 VRM（maid 模型；许可允许商用+二次配布，但须附完整借物表）
...
const FIREFLY_VRM_URL = '/fireflyMaid.vrm' // 萤火女仆：用户导入 VRM，许可允许商用+二次配布，但须附完整借物表
...
export default function Stage({ speakingRef, bubble, onClick, modelSource = 'avaturn' }) {
  ...
  const vrmUrl = modelSource === 'test' ? TEST_VRM_URL : modelSource === 'firefly' ? FIREFLY_VRM_URL : VRM_AVATAR_URL
